...
export default function App() {
  ...
  const {
    ...
    sendMessage,
    setTyping
  } = useSocket(backendUrl); // pass backend URL to useSocket
      ...
      {!joined ? (
        ...
        <div className="flex flex-1 overflow-hidden">
          <aside className="w-64 bg-white border-r p-4 overflow-y-auto">
            ...
            {users.map((u) => (
              <div key={u.id} className="p-1 bg-gray-100 rounded mb-1">
                {u.username}
              </div>
            ))}
          </aside>
              ...
              {messages.map((msg) => (
                <div
                  ...
                  className={`p-2 rounded ${
                    msg.sender === username ? 'bg-blue-500 text-white ml-auto' : 'bg-gray-200'
                  }`}
                >
                  {msg.sender !== username && (
