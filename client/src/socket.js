// socket.js — Socket.io client setup (enhanced version with /clear + history sync)
...
export const useSocket = () => {
  ...
  const sendMessage = (message) => {
    if (message.trim() !== '') socket.emit('send_message', { message });
  };

  // Send a private message
  const sendPrivateMessage = (to, message) => {
    if (message.trim() !== '') socket.emit('private_message', { to, message });
  };
